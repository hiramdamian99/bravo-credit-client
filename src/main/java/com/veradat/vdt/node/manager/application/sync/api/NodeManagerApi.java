...
public interface NodeManagerApi {
     ...
     * @return the process
     */
    @GetMapping(value = "/node-mapping/mapping/{enqueryNodeId}",  produces = MediaType.APPLICATION_JSON_VALUE,
            consumes = MediaType.APPLICATION_JSON_VALUE)
    @ApiResponse(useReturnTypeSchema = true, responseCode = "200", description = "successful to get enquery node")
    ResponseEntity<Mapping> getProcess( @RequestHeader HttpHeaders header,
            @PathVariable String enqueryNodeId) throws VeradatException;
