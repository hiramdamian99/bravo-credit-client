...
public class NodeManagerController implements NodeManagerApi
{
    ...
    public ResponseEntity<List<NodeMapping>> postNodeMappingRequest(@RequestBody NodeMappingCommand command) {
        IdentifierManager.registerMethodIdentifier("postNodeMappingRequest","PNMR");
        logger.info("postNodeMappingRequest");
        List<NodeMapping> mappings = nodeMappingAsyncInputPort.createNodeMapping(command.getOriginNodeId(), command.getProcessId(), command.getNodes());
        HttpStatus status = mappings.isEmpty() ? HttpStatus.NO_CONTENT : HttpStatus.OK;
        return new ResponseEntity<>(mappings, status);
    }

    @Override
    public ResponseEntity<NodeMapping> getNode(@RequestBody String originNode) {
    ...
    public ResponseEntity<Mapping> getProcess(@RequestBody String enqueryNodeId) {
    ...
    @Override
    public ResponseEntity<KeyResponseDTO> getPublicKey(String nodeMappingId, boolean isConversationOrigin, String processType) throws NotFoundException {
        IdentifierManager.registerMethodIdentifier("getPublicKey","GPK");
        logger.info("Iniciando el proceso de consultar la llave pública con los datos: {}, {}, {}", nodeMappingId, isConversationOrigin, processType);
        KeyResponseDTO keyresponse = nodeMappingAsyncInputPort.getKeyAlias(nodeMappingId, isConversationOrigin, processType);
        if (keyresponse == null) {
